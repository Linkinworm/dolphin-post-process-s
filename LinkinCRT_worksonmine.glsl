...
void main()
{
    ...
    if (B_BEZEL > 0.5) {
        if (d_outer > 0.0) {
            ...
            // Uses uv_raw (not warpedUV) so the grille is always geometrically
            // flat — it does not bend with the screen curvature setting.
            if (A_00_FORCE_43 > 0.5) {
                ...
                if (inLeftPillar || inRightPillar) {
                    ...
                    // uv_raw used throughout so grille geometry is always flat
                    float stripX = inLeftPillar
                        ? uv_raw.x / pillarMargin
                        : (1.0 - uv_raw.x) / pillarMargin;
