...
void main()
{
    ...
    // _______________________________________________________________________
    float d_outer = getSDist(warpedUV, B_01_CORNER_R, baseInset);
    float d_game  = getSDist(warpedUV, B_01_CORNER_R, totalInset);
    ...
    if (B_BEZEL > 0.5) {
        if (d_outer > 0.0) {
            ...
            // Subtle screen reflection on bezel
            float2 toCenter   = 0.5 - warpedUV;
            float  distToEdge = max(abs(toCenter.x), abs(toCenter.y)) - (0.5 - totalInset);

            if (distToEdge > 0.0 && distToEdge < 0.05) {
                float2 reflectUV = warpedUV;

                // Push toward nearest screen edge
                if (abs(toCenter.x) > abs(toCenter.y)) {
                    reflectUV.x = (toCenter.x > 0.0) ? totalInset + 0.05 : 1.0 - totalInset - 0.05;
                } else {
                    reflectUV.y = (toCenter.y > 0.0) ? totalInset + 0.05 : 1.0 - totalInset - 0.05;
                }

                float2 gameReflectUV = (reflectUV - totalInset) / (1.0 - totalInset * 2.0);
                ...
                // Fade reflection based on distance from screen edge
    ...
    if (T_TUBEGLASS > 0.5) {
        ...
        // Local UV with user-controlled position offset applied only here.
        // warpedUV itself is never modified so nothing else is affected.
        float2 reflUV = warpedUV - float2(T_05_POS_X, T_06_POS_Y);
